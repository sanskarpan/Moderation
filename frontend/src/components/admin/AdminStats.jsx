          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="font-bold">{stats.flagged.total}</div>
              </div>
              ...
              <div className="h-1 w-full bg-secondary-200 dark:bg-secondary-800">
                <div className="flex h-full">
                  <div 
                    ...
                    style={{ width: `${(stats.flagged.pending / stats.flagged.total) * 100}%` }}
                    title={`Pending: ${stats.flagged.pending}`}
                  ></div>
                  <div 
                    className="bg-success-500" 
                    style={{ width: `${(stats.flagged.approved / stats.flagged.total) * 100}%` }}
                    title={`Approved: ${stats.flagged.approved}`}
                  ></div>
                  <div 
                    className="bg-danger-500" 
                    style={{ width: `${(stats.flagged.rejected / stats.flagged.total) * 100}%` }}
                    title={`Rejected: ${stats.flagged.rejected}`}
                  ></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-md bg-warning-50 p-2 dark:bg-warning-900">
                  ...
                  <div className="text-sm">{stats.flagged.pending}</div>
                </div>
                <div className="rounded-md bg-success-50 p-2 dark:bg-success-900">
                  ...
                  <div className="text-sm">{stats.flagged.approved}</div>
                </div>
                <div className="rounded-md bg-danger-50 p-2 dark:bg-danger-900">
                  ...
                  <div className="text-sm">{stats.flagged.rejected}</div>
                </div>
              </div>
            </div>
          </CardContent>
